  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans">
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 px-6 py-4 flex items-center justify-between shadow-sm">
        ...
        {file && (
          <button onClick={reset} className="text-sm text-slate-500 hover:text-brand-600 flex items-center transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-140px)]">
            <div className="bg-slate-200 rounded-xl overflow-hidden shadow-inner border border-slate-300 relative flex flex-col">
              ...
              <div className="flex-1 overflow-auto flex items-center justify-center p-4">
                {file.type === 'application/pdf' ? (
                  ...
                  <img src={filePreview!} alt="Invoice Preview" className="max-w-full max-h-full object-contain shadow-lg" />
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg border border-slate-200 flex flex-col p-6">
              {isProcessing ? (
                ...
                <DataPanel data={invoiceData} onChange={handleDataChange} onConfirm={handleDownloadUBL} />
              ) : null}
            </div>
          </div>
        )}
